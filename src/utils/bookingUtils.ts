...
export const getAvailableSlots = (
  ...
  while (currentHour < 16) {
    ...
    const hasConflict = existingBookings.some(booking => {
      ...
      const conflict = rangesOverlap(slotStartUTC, slotEndUTC, bStartUTC, bEndUTC);
      if (conflict) {
        console.log(`Conflict: ${timeEST} overlaps with ${booking.bookingReference} (${booking.selectedTime})`);
      }
      return conflict;
    });
