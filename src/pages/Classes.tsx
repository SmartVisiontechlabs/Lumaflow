...
export default function Classes() {
  ...
  useEffect(() => {
    ...
    cmsService.getPagesContent()
      .then(data => {
        if (active && data?.classes) {
          setPageConfig(data.classes);
        }
      })
      .catch(err => {
        console.error('Failed to load Classes Page CMS config:', err);
      });
    return () => { active = false; };
  }, []);
  ...
  const services = pageConfig?.services || defaultServices;

  const getIconComponent = (iconName: string) => {
    switch (iconName) {
      case 'Wind': return <Wind className="w-8 h-8 text-[#CBAE73]" />;
      case 'Heart': return <Heart className="w-8 h-8 text-[#CBAE73]" />;
      case 'Sparkles': return <Sparkles className="w-8 h-8 text-[#CBAE73]" />;
      default: return <Sparkles className="w-8 h-8 text-[#CBAE73]" />;
    }
  };
  ...
  return (
    <div className="min-h-screen bg-cream selection:bg-gold/30">
          ...
          >
            <h1 className="font-display text-5xl md:text-8xl text-[#3A3A3A] mb-8 leading-[1.1] tracking-tight">
              {pageConfig?.hero_title ? (
                ...
                <>Your Path to <br /><span className="italic text-[#CBAE73]">Inner Alignment</span></>
              )}
            </h1>
            <p className="text-xl md:text-2xl text-[#3A3A3A]/60 font-light max-w-2xl mx-auto leading-relaxed italic">
              {pageConfig?.hero_subtitle || `"Choose the practice that meets you where you are."`}
            </p>
          </motion.div>
        </div>
      </section>
      ...
      {/* DETAILED SECTIONS */}
      <section className="space-y-32 pb-36">
        {services.map((service: any, idx: number) => {
          const layout = idx % 2 === 0 ? 'left' : 'right';
          return (
            <div key={service.title} className="px-6">
              <div className={`max-w-7xl mx-auto flex flex-col ${layout === 'right' ? 'lg:flex-row-reverse' : 'lg:flex-row'} items-center gap-16 lg:gap-32`}>
                {/* Image Side */}
                <motion.div
      ...
      {/* FINAL CTA */}
      <section className="py-36 px-6 bg-[#F8F5F0] relative overflow-hidden">
          ...
          >
            <h2 className="font-display text-5xl md:text-7xl text-[#3A3A3A] mb-8 leading-tight">
              {pageConfig?.cta_title ? (
                pageConfig.cta_title.split(pageConfig.cta_title.includes('\\n') ? '\\n' : '\n').map((line: string, i: number, arr: string[]) => (
                  <React.Fragment key={i}>
                    {i === 1 ? <span className="italic text-[#CBAE73]">{line}</span> : line}
                    {i < arr.length - 1 && <br />}
                  </React.Fragment>
                ...
                <>Begin your <br /><span className="italic text-[#CBAE73]">journey inward</span></>
              )}
            </h2>
            <p className="text-lg text-[#3A3A3A]/50 font-light mb-16 max-w-xl mx-auto">
              {pageConfig?.cta_description || "Spaces are intentionally limited to preserve the sacred nature of each container. Reserve your spot in our next group session."}
            </p>
            <button
              onClick={openBooking}
            ...
            >
              {pageConfig?.cta_button_text || "Book Your Session"}
            </button>
          </motion.div>
        </div>
      </section>
