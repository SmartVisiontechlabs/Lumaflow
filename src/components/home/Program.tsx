        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          {isLoading ? (
            Array.from({ length: 3 }).map((_, index) => (
              <div
                key={index}
                className="p-16 rounded-[3rem] bg-[#FAF8F5]/85 border border-[#CBAE73]/15 flex flex-col items-center text-center relative animate-pulse h-[320px]"
              >
                <div className="h-8 w-40 bg-gold/15 rounded mb-6 mt-4" />
                <div className="h-4 w-full bg-gold/10 rounded mb-3" />
                <div className="h-4 w-5/6 bg-gold/10 rounded mb-3" />
                <div className="h-4 w-2/3 bg-gold/10 rounded mb-6" />
                <div className="h-4 w-28 bg-gold/15 rounded-full mt-auto mb-4" />
              </div>
            ))
          ) : (
            offerings.map((program, index) => {
              const benefitText = program.description || program.benefit || '';
              return (
                <motion.div
